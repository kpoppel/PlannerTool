...
test.describe('Feature Card dim-info e2e', () => {
  test('shows dim-info and dims capacity when parent has children', async ({ page }) => {
    ...
    await page.route('**/api/scenario*', async (route) => {
      ...
      if (route.request().method() === 'GET') {
        ...
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ ok: true, id: 'seed' }),
        });
      }
    });
    ...
    await page.route('**/api/tasks', async (route) => {
      const payload = [
        ...
        },
        {
          id: 'child-1',
          title: 'Child One',
          relations: [{ type: 'Parent', id: 'epic-with-child' }],
          type: 'feature',
          start: '2025-01-03',
          end: '2025-01-10',
        },
    ...
    const requests = [];
    page.on('request', (req) => {
      if (req.url().includes('/api/tasks')) requests.push(req.url());
    });
    ...
    if (!epicWithChild)
      throw new Error(
        'epic-with-child not found in DOM results: ' + JSON.stringify(results)
      );
    if (!epicNoChild)
      throw new Error(
        'epic-no-child not found in DOM results: ' + JSON.stringify(results)
      );
    if (!epicWithChild.hasDim || !epicWithChild.teamDimmed)
      throw new Error(
        'epic-with-child expected dimmed but was not: ' + JSON.stringify(epicWithChild)
      );
    if (epicNoChild.hasDim || epicNoChild.teamDimmed)
      throw new Error(
        'epic-no-child should not be dimmed: ' + JSON.stringify(epicNoChild)
      );
  });
});
