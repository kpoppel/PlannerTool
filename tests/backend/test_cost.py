...
def test_calculate_exact_values_permutations():
    ...
    for name, caps in capacity_sets.items():
        for days in days_list:
            ...
            assert abs(res['internal_hours'] - exp['internal_hours']) < 1e-2, f"{name} {days} internal_hours"
            assert abs(res['external_hours'] - exp['external_hours']) < 1e-2, f"{name} {days} external_hours"
            assert abs(res['internal_cost'] - exp['internal_cost']) < 1e-2, f"{name} {days} internal_cost"
            assert abs(res['external_cost'] - exp['external_cost']) < 1e-2, f"{name} {days} external_cost"
