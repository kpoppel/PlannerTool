...
# is not reachable.
# The `app` fixture in `tests/conftest.py` prepares an in-memory storage
# and ensures `config/server_config` exists. Do not perform app creation
# at import time here; instead use the `app` fixture in tests.
...
def test_api_endpoint_session_behaviour(server_available, base_url: str, app):
