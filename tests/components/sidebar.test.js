...
describe('Sidebar Consolidated Tests', () => {
  ...
  beforeEach(async () => {
    await customElements.whenDefined('plan-menu');
    await customElements.whenDefined('team-menu');
  });
  ...
  it('renders open by default and hides when open=false', async () => {
    ...
    const closed = root.querySelector('.sidebar.closed');
  ...
  it('project and team All/None toggles work', async () => {
    ...
    let planMenu =
      root2.querySelector('plan-menu') || document.querySelector('plan-menu');
    if (!planMenu) {
      planMenu = document.createElement('plan-menu');
      document.body.appendChild(planMenu);
    }
    // Ensure the menu always reflects the current state's projects to avoid stale shared instances
    if (planMenu) {
      planMenu.projects = state.projects;
    }
    ...
    expect(planMenu._anyUncheckedProjects()).to.equal(
      true,
      'planMenu reports no unchecked projects before toggle'
    );
    expect(state.projects.some((p) => !p.selected)).to.equal(
      true,
      'state reports no unchecked projects before toggle'
    );
    ...
    const waitProjectChanged2 = new Promise((resolve) => {
      const handler = () => {
        bus.off(ProjectEvents.CHANGED, handler);
        projectChanged2 = true;
        resolve();
      };
      bus.on(ProjectEvents.CHANGED, handler);
    });
    pbtn.click();
    await waitProjectChanged2;
    expect(projectChanged2).to.be.true;
    expect(state.projects.every((p) => !p.selected)).to.be.true;

    bus.emit(TeamEvents.CHANGED, state.teams);
    let teamMenu =
      root2.querySelector('team-menu') || document.querySelector('team-menu');
    if (!teamMenu) {
      teamMenu = document.createElement('team-menu');
      document.body.appendChild(teamMenu);
    }
    // Ensure created team menu reflects current teams (overwrite stale instances)
    if (teamMenu) {
      teamMenu.teams = state.teams;
    }
    ...
    const tbtn = tmRoot.querySelector('.list-toggle-btn');
    expect(tbtn).to.exist;
    ...
    tbtn.click();
    await waitTeamChanged;
    expect(teamChanged).to.be.true;
    ...
    tbtn.click();
    await waitTeamChanged2;
    expect(teamChanged2).to.be.true;
