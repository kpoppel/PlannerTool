...
describe('Timeline Consolidated Tests', () => {
  it('renders header with month cells and exposes APIs', async () => {
    ...
    const el = await fixture(
      html`<timeline-lit .months=${months} .bus=${bus}></timeline-lit>`
    );
    await el.updateComplete;
    const cells = el.shadowRoot.querySelectorAll('.timeline-cell');
    const header =
      document.getElementById('timelineHeader') || document.createElement('div');
    header.id = 'timelineHeader';
    document.body.appendChild(header);
    const section =
      document.getElementById('timelineSection') || document.createElement('div');
    ...
    const totalWidth = el.getTotalWidth();
    expect(totalWidth).to.be.a('number');
  });

  it('sets month cell widths correctly and calculates total width', async () => {
    const months = [new Date('2025-01-01'), new Date('2025-02-01')];
    const el = await fixture(
      html`<timeline-lit .months=${months} .monthWidth=${120} .bus=${bus}></timeline-lit>`
    );
    await el.updateComplete;
  ...
  it('renderMonths updates DOM and handles empty months', async () => {
    ...
    const el = await fixture(
      html`<timeline-lit .months=${initialMonths} .bus=${bus}></timeline-lit>`
    );
    await el.updateComplete;
    ...
    const empty = await fixture(
      html`<timeline-lit .months=${[]} .bus=${bus}></timeline-lit>`
    );
  ...
  it('computes months and emits MONTHS event via initTimeline', async () => {
    ...
    const { _resetTimelineState, initTimeline } =
    ...
    await new Promise((resolve, reject) => {
      bus.on(TimelineEvents.MONTHS, (months) => {
        try {
          ...
          expect(months.length).to.be.at.least(3);
          resolve();
        } catch (e) {
          reject(e);
        }
    ...
    ['timelineHeader', 'timelineSection', 'featureBoard'].forEach((id) => {
