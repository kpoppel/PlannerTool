...
export function aggregateDailyLoads(features, teams, projects, flags, range){
  ...
  for(const item of features){
    ...
    if(isEpic){
      ...
      for(let d = Math.max(startIdx, itemStartIdx); d <= Math.min(endIdx, itemEndIdx); d++){
        ...
        for(const tl of item.teamLoads){ if(!teamSet.has(tl.team)) continue; add(d, tl.team, tl.load); }
      }
    } else {
...
export function computeOrganisationalLoad(dayBucket){
...
// Extended computation producing normalized per-day team and project loads.
...
export function computeDailyLoadMaps(features, teams, projects, flags, range){
  const rawTeamDayMap = aggregateDailyLoads(features, teams, projects, flags, range);
  const numTeamsGlobal = teams.length === 0 ? 1 : teams.length; // avoid div by zero
  const teamDayMap = new Map(); // normalized team loads (percent of org capacity)
  const projectDayMap = new Map(); // normalized project loads (aggregate of participating teams per project)
  ...
  // Precompute timeline months & range indices for per-day iteration like aggregateDailyLoads did
  const months = getTimelineMonths();
  function dateToIndex(months, date){
    const start = months[0];
    const msPerDay = 24*60*60*1000;
  ...
  for(const item of features){
    ...
    if(isEpic){
      ...
      for(let d=Math.max(startIdx,itemStartIdx); d<=Math.min(endIdx,itemEndIdx); d++){
        const coveredByChild = showFeatures && childRanges.some(r=>d>=r.s && d<=r.e);
        if(coveredByChild) continue;
        for(const tl of item.teamLoads){
          if(!teamSetSelected.has(tl.team)) continue; // respect team filter also in project view per requirement
          addNormalizedTeam(d, tl.team, tl.load);
          addNormalizedProject(d, item.project, tl.load);
        }
      }
    } else {
      if(!showFeatures) continue;
