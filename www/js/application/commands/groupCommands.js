...
export function createLegacyGroupCommands(state, groupService) {
  function legacyAddMemberToGroup(groupId, taskId) {
    const scenario = state.getActiveScenario?.();
    if (!scenario || !groupId || !taskId) return false;

    const localGroup = (scenario.scenarioGroups || []).find(
      (group) => String(group.id) === String(groupId)
    );
    if (localGroup) {
      const members = Array.isArray(localGroup.members) ? localGroup.members : [];
      if (members.some((memberId) => String(memberId) === String(taskId))) return true;
      localGroup.members = [...members, String(taskId)];
      state._markActiveScenarioChanged?.();
      return true;
    }

    state.applyGroupMemberDelta?.(String(groupId), String(taskId), 'add');
    return true;
  }

  function legacyRemoveMemberFromGroup(groupId, taskId) {
    const scenario = state.getActiveScenario?.();
    if (!scenario || !groupId || !taskId) return false;
    ...
    if (localGroup) {
      localGroup.members = (localGroup.members || []).filter(
    ...
    state.applyGroupMemberDelta?.(String(groupId), String(taskId), 'remove');
    return true;
  }

  return {
    createGroupInScenario(planId, name, color = null, parentId = null) {
      return state.createGroupInScenario(planId, name, color, parentId);
    },

    updateGroupInScenario(groupId, fields) {
      return state.updateGroupInScenario(groupId, fields);
    },

    deleteGroupInScenario(groupId) {
      return state.deleteGroupInScenario(groupId);
    },

    applyGroupMemberDelta(groupId, taskId, op) {
      return state.applyGroupMemberDelta(groupId, taskId, op);
    },

    addMemberToGroup(groupId, taskId) {
      return legacyAddMemberToGroup(groupId, taskId);
    },

    removeMemberFromGroup(groupId, taskId) {
      return legacyRemoveMemberFromGroup(groupId, taskId);
    },

    clearPendingGroupChanges() {
      return state.clearPendingGroupChanges();
    },

    confirmGroupCreate(tempId, realId) {
      return state.confirmGroupCreate(tempId, realId);
    },
  };
}
