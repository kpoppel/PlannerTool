...
export class QueuedFeatureService {
  ...
  updateFeatureDates(updates, capacityCallback) {
    ...
    for (const u of updates) {
      if (u && u.id) this._updateQueue.push(u);
    }
    if (typeof capacityCallback === 'function')
  ...
  _scheduleProcessQueue() {
    ...
    const process = () => {
      ...
      for (const u of items) {
        if (u && u.id && u.start !== undefined && u.end !== undefined)
          ...
          if (childIds.length) {
            ...
            if (epicQueued && epicQueued.start) {
              try {
                ...
                const activeEpicOv = activeScenario.overrides[base.id] || {
                  start: base.start,
                  end: base.end,
                };
                const activeEpicStart = activeEpicOv.start || base.start;
                epicMovedDeltaMs =
                  Date.parse(epicQueued.start) - Date.parse(activeEpicStart);
            ...
            for (const cid of childIds) {
              ...
              if (epicMovedDeltaMs !== null) {
                ...
                const childOv2 = activeScenario.overrides[cid] || {
                  start: chBase.start,
                  end: chBase.end,
                };
              ...
              const ovActive =
                activeScenario.overrides ? activeScenario.overrides[cid] : undefined;
              const effStart =
                ovNew && ovNew.start ? ovNew.start
                : ovActive && ovActive.start ? ovActive.start
                : chBase.start;
              const effEnd =
                ovNew && ovNew.end ? ovNew.end
                : ovActive && ovActive.end ? ovActive.end
                : chBase.end;
              if (maxChildEnd === null || effEnd > maxChildEnd) maxChildEnd = effEnd;
              if (minChildStart === null || effStart < minChildStart)
                minChildStart = effStart;
            }
            ...
            const candidateStart =
              start !== undefined && start !== null ? start : baselineEpicStart;
            const candidateEnd =
              end !== undefined && end !== null ? end : baselineEpicEnd;
            ...
            if (epicQueued) {
              ...
              const endCandidates = [baselineEpicEnd, candidateEnd, maxChildEnd].filter(
                Boolean
              );
              start =
                startCandidates.length ?
        ...
        if (base.type === 'feature' && base.parentEpic) {
          ...
          if (epicBase) {
            ...
            if (changed) {
              ...
              if (!appliedIds.includes(epicId)) appliedIds.push(epicId);
            }
          }
        }
      }
