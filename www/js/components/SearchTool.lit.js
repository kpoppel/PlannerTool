...
export class SearchTool extends LitElement {
          ...
          ${this.results.map(
            (r, idx) => html`
              <div
                class="result"
                role="option"
                aria-selected="${this.highlightIndex === idx}"
                @click="${() => this._onSelect(r)}"
                @mouseover="${() => this._setHighlight(idx)}"
              >
  ...
  open() {
    ...
    const stored = (function () {
      try {
        return localStorage.getItem('az_planner:search:lastQuery');
      } catch (e) {
        return null;
      }
    })();
  ...
  _onKeyDown(e) {
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      const r = this.results[this.highlightIndex] || this.results[0];
      if (r) this._onSelect(r);
  ...
  _performSearch() {
    ...
    for (const f of all) {
      ...
      if (fid.indexOf(q) !== -1) {
        idMatches.push({ id: fid, title });
        continue;
      }
      if (title.toLowerCase().indexOf(qLower) !== -1) {
        titleMatches.push({ id: fid, title });
      }
    }
  ...
  _onSelect(item) {
    try {
      ...
      console.warn('SearchTool: center failed', e);
    }
