...
export class ColorPopoverLit extends LitElement {
  ...
  setPalette(p) {
    this.palette = Array.isArray(p) ? p.slice() : [];
    this.requestUpdate();
  }
  ...
  render() {
    ...
    const palette =
      Array.isArray(this.palette) && this.palette.length ? this.palette : [];
    return html`<div class="color-popover" style="display:${display};">
      ${palette.map(
        (c) =>
          html`<button
            class="color-swatch"
