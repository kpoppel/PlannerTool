...
export class HelpModal extends LitElement {
  ...
  async _loadIndex() {
    try {
      ...
      console.error('[HelpModal] failed to fetch index.json', e);
      this.index = [];
    }
    // Auto-select first document if available, otherwise show fallback message
    if (this.index.length) {
      try {
        await this._selectDoc(this.index[0]);
      } catch (e) {
        console.error('[HelpModal] _selectDoc error', e);
        this.content = '<div class="empty">Could not load the first document.</div>';
      }
    } else {
      this.content =
        '<div class="empty">No documentation found. Add files to <code>www/docs/</code> and update <code>index.json</code>.</div>';
    }
  ...
  async _selectDoc(doc) {
    ...
    try {
      ...
      this.content = '<div class="empty">Could not load document.</div>';
    }
    this.requestUpdate();
    ...
    for (let line of lines) {
      if (line.startsWith('```')) {
        if (!inCode) {
          ...
          html += `<pre><code class="lang-${this._escapeHtml(codeLang)}">`;
        } else {
          inCode = false;
          html += '</code></pre>';
        }
      ...
      if (h) {
        ...
        html += `<h${level}>${this._inline(h[2])}</h${level}>`;
        continue;
      }
  ...
  _inline(text) {
    ...
    text = text.replace(
      /\*\*([^*]+)\*\*/g,
      (m, c) => `<strong>${this._escapeHtml(c)}</strong>`
    );
    // italic *text*
  ...
  firstUpdated() {
    ...
    if (panel) {
      panel.addEventListener('click', (e) => {
        ...
        try {
          this._selectDoc({ file });
        } catch (err) {
          console.warn('[HelpModal] failed to open internal link', err);
        }
      });
    }
  }
    ...
    return html`
      <modal-lit ?open=${this.open}>
          ...
          <div class="help-list">
            ...
            <ul>
              ${list.map(
                (d) =>
                  html`<li
                    class=${this.current && this.current.file === d.file ? 'active' : ''}
                    @click=${() => this._selectDoc(d)}
                  >
                    ${d.title}
                  </li>`
              )}
        ...
        <div slot="footer" class="modal-footer">
          <button class="btn secondary" @click=${() => this._showOnboarding()}>
            Show Onboarding
          </button>
          <button
            class="btn"
            @click=${() => {
              const m = this.renderRoot.querySelector('modal-lit');
              try {
                if (m) m.close();
              } catch (e) {
                this._onModalClose();
