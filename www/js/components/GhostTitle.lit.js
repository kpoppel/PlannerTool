...
class GhostTitle extends LitElement {
  ...
  updated(changedProperties) {
    ...
    if (
      changedProperties.has('cardRect') ||
  ...
  _performPositioning() {
    ...
    const cardLeftVisible =
      cardLeftInViewport >= 0 && cardLeftInViewport <= viewportWidth;
    const cardRightVisible =
      cardRightInViewport >= 0 && cardRightInViewport <= viewportWidth;
    ...
    if (isCardOnScreen) {
      ...
      if (cardRightInViewport < 0) {
        ...
        ghostLeft = cardLeft - ghostWidth - gap;
        if (ghostLeft < scrollLeft + gap) ghostLeft = scrollLeft + gap;
        else isStuck = false;
      }
      ghostTop = cardTop + cardHeight / 2 - ghostHeight / 2;
    }
