...
export class ToolsMenuLit extends LitElement {
  ...
  _onPluginClick(e, p) {
    ...
    try {
      ...
      console.warn('[ToolsMenu] plugin click error', ex);
    }
  }
        ...
        <ul class="sidebar-list">
          ${Array.isArray(this.plugins) && this.plugins.length > 0 ?
            this.plugins.map((p) => {
              const isActive =
                pluginManager &&
                typeof pluginManager.isActive === 'function' &&
                pluginManager.isActive(p.id);
              return html` <li class="sidebar-list-item">
                <div
                  class="chip sidebar-chip ${isActive ? 'active' : ''} ${p.disabled ?
                    'disabled'
                  : ''}"
                  role="button"
                  tabindex="0"
                  @click=${(e) => {
                    if (!p.disabled) this._onPluginClick(e, p);
                  }}
                  title=${p.description || ''}
                >
                  ...
                  ${p.version ?
                    html`<div class="chip-badge small">${p.version}</div>`
                  : ''}
                </div>
              </li>`;
            })
          : html`<li style="padding:8px;color:var(--color-sidebar-text);opacity:0.8;">
              No tools available
            </li>`}
        </ul>
      </div>
