...
export class PluginHistoryComponent extends LitElement {
    ...
    return this.visible ? html`
      <div class="floating-toolbar">
        ...
        ${!this.loading && taskCount > 0 ? html`
          ...
          <div class="task-count" style="font-size: 11px; margin-top: 4px; opacity: 0.8;">
  ...
  async refresh(invalidateCache = false) {
    ...
    try {
      ...
      const projectsToFetch = invalidateCache 
      ...
      console.log(`[PluginHistory] Tasks with actual history entries: ${tasksWithHistory.length}`);
    } catch (err) {
  ...
  _renderTaskHistory(taskData, months, monthWidth, boardOffset) {
    ...
    if (!cardEl) {
      console.debug(`[PluginHistory] Could not find card for task ${taskId} (${taskData.title})`);
      return;
    }
  ...
  _drawHistoryLine(historyEntries, y, color, months, monthWidth, boardOffset, taskTitle, fieldType) {
    ...
    console.debug(`[PluginHistory] Drawing ${fieldType} line with ${historyEntries.length} entries at y=${y}`);
    ...
    const points = historyEntries
      .map((entry, idx) => {
        ...
        if (x === null) {
          console.debug(`[PluginHistory] Skipping ${fieldType} entry (out of timeline range):`, entry.value, date);
        }
        return x !== null ? { x, entry, idx } : null;
      })
      .filter(p => p !== null);
    
    console.debug(`[PluginHistory] ${points.length}/${historyEntries.length} points are within timeline range`);
    ...
    points.forEach(({ x, entry, idx }) => {
      ...
      const showTooltip = (event) => {
        ...
        const tooltipContent = [
          ...
          isFirst ? '(original)' : isLast ? '(current)' : ''
  ...
  _drawFishboneConnectors(history, cardY, cardHeight, months, monthWidth, boardOffset) {
    ...
    pairMap.forEach((pair, pairId) => {
      ...
      const startChangedAt = startEntry.changed_at ? new Date(startEntry.changed_at) : null;
      ...
      const startX = this._calcX(new Date(startEntry.value), months, monthWidth, boardOffset);
