 ...
 * `plugin-cost-v1` component and coordinates initial data loading.
 *
...
class PluginCostV1Plugin {
  constructor(id = 'plugin-cost-v1', config = {}) {
    this.id = id;
    this.config = config;
    this._el = null;
    this._host = null;
  ...
  async init() {
    ...
    if (!this._componentLoaded) {
      await import('./PluginCostV1Component.js');
  ...
  async activate() {
    ...
    if (!this._el) {
      this._el = document.createElement('plugin-cost-v1');
...
export default PluginCostV1Plugin;
