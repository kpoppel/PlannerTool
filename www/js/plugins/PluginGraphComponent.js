...
export class PluginGraph extends LitElement {
  ...
  _ensureSvg() {
    ...
    if (!this.svgEl) {
      ...
      this.svgEl.addEventListener('pointermove', (e) => {
        ...
        let lo = 0,
          hi = this.dayOffsets.length - 1;
        while (lo < hi) {
          const mid = Math.floor((lo + hi) / 2);
          if (this.dayOffsets[mid] <= unscaledX) lo = mid + 1;
          else hi = mid;
        }
        const dayIdx = Math.max(0, lo - 1);
        if (dayIdx >= 0 && dayIdx < this.dayOffsets.length - 1) {
          this._showTooltip(
            dayIdx,
            this.lastRenderedData && this.lastRenderedData.totals ?
              this.lastRenderedData.totals[dayIdx]
            : null
          );
          this._moveTooltip(e);
        }
  ...
  exportPng() {
    ...
    img.onload = () => {
      const host = this.renderRoot.getElementById('mountainViewHost');
      const vb =
        this.svgEl.viewBox && this.svgEl.viewBox.baseVal ?
          this.svgEl.viewBox.baseVal
        : null;
  ...
  _renderAxes(width, height, sDate, eDate, maxY = 100, step = 20) {
    ...
    for (let i = 0; i <= numTicks; i++) {
      ...
      label.setAttribute('y', String(yPos - 2));
    ...
    while (cursor <= end) {
      const dayIndex = Math.max(
        0,
        Math.min(totalDays - 1, this._daysBetween(sDate, cursor) - 1)
      );
  ...
  _xForDayGlobal(i) {
    if (this.dayOffsets && i >= 0 && i < this.dayOffsets.length)
      return Math.floor(this.dayOffsets[i]);
    return 0;
  }
  ...
  _moveTooltip(e) {
    ...
    if (left - window.scrollX + tipW + 8 > winW)
      left = Math.max(window.scrollX + 4, window.scrollX + winW - tipW - 8);
    if (top - window.scrollY + tipH + 8 > winH)
      top = Math.max(window.scrollY + 4, window.scrollY + winH - tipH - 8);
    this.tooltipEl.style.left = `${left}px`;
    this.tooltipEl.style.top = `${top}px`;
  }
  ...
  _render() {
    ...
    const hContent = host.clientHeight;
    const bottomPad = 56;
    this.svgEl.setAttribute('viewBox', `0 0 ${w} ${hContent + bottomPad}`);
    const data = this._computeDailyTotals(this.mode, this.startDate, this.endDate);
    console.debug('[plugin-graph] render', {
      mode: this.mode,
    ...
    if (!data || !data.days || data.days <= 0) {
      this._renderAxes(w, hContent, this.startDate, this.endDate, 100, 20);
      return;
    }
    ...
    for (let i = 0; i < daysCount; i++) {
      const p = monthWidth / daysInMonth(cur);
      cum += p;
      this.dayOffsets[i + 1] = cum;
      cur.setDate(cur.getDate() + 1);
    }
    const totalContentWidth = this.dayOffsets[this.dayOffsets.length - 1] || 1;
    this.xScale = w / totalContentWidth;
