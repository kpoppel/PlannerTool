...
export function renderTaskView(component) {
  ...
  if (!component._expandedSections)
    component._expandedSections = new Set(
      Array.from(featuresByProject.keys()).map((k) => `project-${k}`)
    );
        ...
        return html`
          <div style="margin-bottom:20px;">
            <div
              ...
              @click="${() => {
                if (!component._expandedSections) component._expandedSections = new Set();
                if (component._expandedSections.has(key))
                  component._expandedSections.delete(key);
                else component._expandedSections.add(key);
            ...
            ${isExpanded ?
              html`<div style="margin-left:12px; margin-top:8px;">
                ${renderTasksInProjectTable(component, projectFeatures)}
              </div>`
            : ''}
          </div>
        `;
...
function renderTasksInProjectTable(component, features) {
  if (!features || features.length === 0) {
    return html`<div style="color:#666; font-size:13px; margin-bottom:12px;">
      No tasks available for the selected projects.
    </div>`;
  }
    ...
    <div style="${indentStyle}">
      ...
      >
        ...
        ${hasDeviation ?
          html`
            <span
              class="deviation-indicator ${Math.abs(deviationPercent) > 20 ?
                'high'
              : 'medium'}"
            >
        ...
        <span style="margin-left:16px; color:#666; font-size:12px;">
          ${component.viewMode === 'cost' ?
            `Cost: ${totalCost.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
        ...
        <tbody>
          <tr>
            ...
            <td class="numeric">
              ${formatValue(
                component.viewMode === 'cost' ?
                  deviation.parentOwn.internalCost
                : deviation.parentOwn.internalHours
              )}
            </td>
            <td class="numeric">
              ${formatValue(
                component.viewMode === 'cost' ?
                  deviation.parentOwn.externalCost
                : deviation.parentOwn.externalHours
              )}
            </td>
            <td class="numeric">
              ${formatValue(
                component.viewMode === 'cost' ?
                  deviation.parentOwn.totalCost
                : deviation.parentOwn.totalHours
              )}
            </td>
            <td></td>
          </tr>
          <tr>
            ...
            <td class="numeric">
              ${formatValue(
                component.viewMode === 'cost' ?
                  deviation.childrenSum.internalCost
                : deviation.childrenSum.internalHours
              )}
            </td>
            <td class="numeric">
              ${formatValue(
                component.viewMode === 'cost' ?
                  deviation.childrenSum.externalCost
                : deviation.childrenSum.externalHours
              )}
            </td>
            <td class="numeric">
              ${formatValue(
                component.viewMode === 'cost' ?
                  deviation.childrenSum.totalCost
                : deviation.childrenSum.totalHours
              )}
            </td>
            <td></td>
          </tr>
          <tr style="font-weight:600;">
            ...
            <td class="numeric">
              ${deviation.deviation[
                component.viewMode === 'cost' ? 'internalCost' : 'internalHours'
              ].toFixed(0)}%
            </td>
            <td class="numeric">
              ${deviation.deviation[
                component.viewMode === 'cost' ? 'externalCost' : 'externalHours'
              ].toFixed(0)}%
            </td>
            <td class="numeric">
              ${deviation.deviation[
                component.viewMode === 'cost' ? 'totalCost' : 'totalHours'
              ].toFixed(0)}%
            </td>
            <td></td>
          </tr>
