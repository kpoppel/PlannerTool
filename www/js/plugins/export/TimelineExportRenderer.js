...
export class TimelineExportRenderer {
  ...
  async exportToPng(options = {}) {
    ...
    try {
      // Build the SVG
      const svg = await this.buildExportSvg({
        includeAnnotations,
        includeDependencies,
        scrollLeft,
        scrollTop,
  ...
  async buildExportSvg(options = {}) {
    ...
    this._height = Math.floor(
      viewport.mainGraphHeight + TIMELINE_HEADER_HEIGHT + viewport.fullHeight
    );
    ...
    for (const featureObj of features) {
      ...
      if (visibleWidth > 40) {
        ...
        const availableWidth =
          visibleWidth - CARD_PADDING * 2 - CARD_BORDER_LEFT_WIDTH - iconReserve;
        ...
        const titleText = createSvgText(truncatedTitle, textX, cardY + cardHeight / 2, {
  ...
  _renderGhostTitles(yOffset, viewport) {
    ...
    for (const cardData of this._cardData) {
      ...
      const cardRightVisible =
        cardRightInViewportX >= 0 && cardRightInViewportX <= this._width;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const lineText = createSvgText(lines[i], textX, lineY, {
    ...
    for (const f of allFeatures) {
      ...
      for (const rel of relations) {
        ...
        if (relType === 'Successor') {
          ...
          from = {
            x: otherPos.left + otherPos.width,
            y: otherPos.top + otherPos.height / 2,
          };
          to = { x: targetPos.left, y: targetPos.top + targetPos.height / 2 };
        } else {
          from = {
            x: otherPos.left + otherPos.width / 2,
            y: otherPos.top + otherPos.height / 2,
          };
          to = {
  ...
  _renderNoteAnnotation(ann, xOffset = 0, yOffset = 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const text = createSvgText(lines[i], x + 6, startY + i * lineHeight, {
