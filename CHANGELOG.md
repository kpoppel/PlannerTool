...
### Changed
- Iterations backend now supports rule-based v2 resolution (with legacy config compatibility), adding per-project resolution metadata (`matchedRuleId`, `fallbackUsed`, `resolutionWarnings`) to iteration groups.
...
- Scenario lists and active IDs now read from AppStore after composition, and remote scenario data commits through a command before scenario effects emit.
- Removed the disabled experimental queued feature service, its runtime feature flag, and dedicated tests, eliminating a duplicate asynchronous scenario-mutation path.
- AppStore is now canonical for project/team selection and view expansion: commands commit those slices once, while runtime services derive selected flags for effects without snapshot reconciliation.
...
- Core event/plugin plumbing simplification: removed legacy string-event warning path, eliminated PluginManager load-order bookkeeping duplication, and simplified service resolution/error wrapping in the DI registry.
- Client service cleanup: removed test-only icon helper exports in favor of the canonical icon API and deleted dead fallback/commented paths from the mock provider implementation.
...
- Cost Plugin V2: Task View and Team View now use hierarchy-aware rollup (buildByTeam), DFS feature ordering, window-restricted sums, and clip indicators for out-of-window dates; shared utilities (flattenTree, buildByTeam, computeEffectiveDataMaps) moved to PluginCostV2Calculator.
...
```
Migrations `0019` (display-mode-field), `0020` (rename flags), `0021` (config → diskcache), `0022` (people.yml → diskcache), and `0023` (clean server config) must be applied on any existing installation.
