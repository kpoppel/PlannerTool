  ...
  try {
    ...
    if (!openBtn) {
      ...
      await page.screenshot({
        path: `tests/e2e/diag-${ts}-no-openbtn.png`,
        fullPage: true,
      });
    ...
    try {
      ...
      await page.screenshot({
        path: `tests/e2e/diag-${ts}-form-visible.png`,
        fullPage: true,
      });
      const html = await page.content();
      ...
      await page.screenshot({
        path: `tests/e2e/diag-${ts}-no-form.png`,
        fullPage: true,
      });
      const html = await page.content();
    ...
    try {
      await page.screenshot({
        path: `tests/e2e/diag-${ts}-error.png`,
        fullPage: true,
      });
    } catch (e) {}
    try {
      fs.writeFileSync(`tests/e2e/diag-${ts}-error.txt`, String(err.stack || err));
    } catch (e) {}
    process.exit(1);
  } finally {
    try {
