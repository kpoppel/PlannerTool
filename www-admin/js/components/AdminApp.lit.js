...
export class AdminApp extends LitElement {
    ...
    return html`<aside class="admin-sidebar">
      ...
      <div role="list">
        ${this.sections.map(
          (s) =>
            html`<div
              class="chip ${s === this.activeSection ? 'active' : ''}"
              role="button"
              tabindex="0"
  ...
  renderContent() {
    switch (this.activeSection) {
      ...
      case 'Teams':
        return html`<admin-teams></admin-teams>`;
      case 'People':
        return html`<admin-people></admin-people>`;
      case 'Cost':
        return html`<admin-cost></admin-cost>`;
      case 'Area Mappings':
        return html`<area-mappings-new></area-mappings-new>`;
      case 'Iterations':
